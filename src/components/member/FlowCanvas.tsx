...
export default function FlowCanvas({ project, nodes, edges, selectedTaskId, onSelectTask }: FlowCanvasProps) {
  ...
  return (
    <div className="flow-canvas">
      ...
      <div className="flow-graph" style={{ minWidth: canvasSize.width, minHeight: canvasSize.height }}>
        ...
        {nodes.map((node) => <FlowNode key={node.id} project={project} task={node} selected={node.id === selectedTaskId} onSelectTask={onSelectTask} />)}
      </div>
    </div>
  );
}
