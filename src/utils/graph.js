export function getConnectedTaskIds(tasks, edges, selectedTaskId) {
  if (!selectedTaskId) return new Set();
  const taskIds = new Set(tasks.map((task) => task.id));
  if (!taskIds.has(selectedTaskId)) return new Set();

  const adjacency = new Map(tasks.map((task) => [task.id, new Set()]));
  edges.forEach((edge) => {
...
export function getTaskSubgraph(tasks, edges, selectedTaskId) {
